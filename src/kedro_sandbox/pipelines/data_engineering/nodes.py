...
def log_running_time(func: Callable) -> Callable:
    ...
    def with_time(*args, **kwargs):
        ...
        log.info(f"Running {func.__name__} took {elapsed:.2f} seconds")
        return result
...
def create_master_table(shuttles: pd.DataFrame, companies: pd.DataFrame, reviews: pd.DataFrame) -> pd.DataFrame:
    """Combines all data to create a master table.
        ...
        Returns:
            Master table.
    """
    rated_shuttles: pd.DataFrame = shuttles.merge(reviews, left_on="id", right_on="shuttle_id")

    with_companies: pd.DataFrame = rated_shuttles.merge(companies, left_on="company_id", right_on="id")
